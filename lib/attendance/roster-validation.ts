...
export const updatePublishedRosterEntrySchema = z.object({ entryId: z.string().trim().min(1), shiftTemplateId: z.string().trim().min(1), expectedUpdatedAt: z.iso.datetime(), reason: z.string().trim().min(8).max(240) });
