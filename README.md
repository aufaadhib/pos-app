...
- `/attendance/roster` menyediakan dua mode per outlet untuk owner/manager: roster mingguan manual serta jadwal tetap Senin–Minggu yang berulang dan dimaterialisasi otomatis. Pola dapat disalin ke beberapa staf; perubahan satu tanggal dapat mengganti shift, menjadi Libur, atau kembali mengikuti pola dengan alasan audit. Roster terbit tetap utuh, satu staf hanya boleh memiliki satu shift per tanggal secara global, dan shift lintas tengah malam mengikuti zona waktu outlet.
...
- Jika tanggal lokal outlet sudah melewati tanggal akhir shift tanpa absen pulang, sesi ditandai **Tidak absen pulang** tanpa membuat waktu pulang. Lock sesi lama dilepas secara atomik saat staf memulai absensi masuk berikutnya; durasi tetap nol sampai owner/manager mengoreksi waktu pulang.
- Foto attempt disimpan pada Vercel Blob private terpisah dan cron maintenance menghapusnya setelah 30 hari sekaligus memastikan roster jadwal tetap minggu berjalan/berikutnya tersedia. Embedding probe tidak disimpan; template aktif dan sampel daftar ulang pending dienkripsi AES-256-GCM. Payload pending dihapus setelah keputusan, sedangkan template lama baru dihapus ketika penggantian disetujui.
...
- Production memerlukan `ATTENDANCE_EMBEDDING_KEY`, `ATTENDANCE_BLOB_READ_WRITE_TOKEN`, dan `CRON_SECRET`. Setelah konfigurasi, validasi koneksi private Blob dan respons `200` dari `/api/cron/attendance-maintenance` sebelum mengaktifkan absensi outlet.
...
- `/attendance/roster` — pilihan roster mingguan atau jadwal tetap berulang pada outlet aktif untuk owner/manager.
