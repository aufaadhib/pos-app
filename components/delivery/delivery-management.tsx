...
function MoneyField({ label, value, onChange, allowNegative = false }: { label: string; value: string; onChange: (value: string) => void; allowNegative?: boolean }) { return <label className="grid gap-2 text-sm font-medium">{label}<Input inputMode="numeric" onChange={(event) => onChange(event.target.value.replace(allowNegative ? /[^\d-]/g : /\D/g, ""))} value={value} /></label>; }
