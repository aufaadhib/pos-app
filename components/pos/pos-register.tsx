...
export function PosRegister({ menu, openOrders = [] }: { menu: PosMenu; openOrders?: OpenOrder[] }) {
  ...
  return (
    <div className="pos-register flex min-h-0 flex-1 overflow-hidden rounded-2xl border bg-card shadow-sm xl:grid xl:grid-cols-[minmax(0,1fr)_22rem]">
      ...
      <Dialog onOpenChange={setMobileCartOpen} open={mobileCartOpen}><DialogContent className="inset-x-3 top-1/2 bottom-auto max-h-[calc(100svh-2rem)] -translate-y-1/2 overflow-hidden p-0 sm:p-0" showCloseButton={false}><DialogHeader className="sr-only"><DialogTitle>Pesanan saat ini</DialogTitle><DialogDescription>Periksa item sebelum pembayaran.</DialogDescription></DialogHeader><CartPanel canSave={Boolean(menu.outlet.openOrdersEnabled && !selectedChannel)} cart={cart} changeQuantity={changeQuantity} currentOrder={currentOrder} modal onCheckout={() => setCheckoutOpen(true)} onEditNote={setNoteTarget} onRemove={(id) => setCart((current) => current.filter((line) => line.id !== id))} onSave={() => setSaveOrderOpen(true)} onSend={sendCurrentOrder} pending={orderPending} totals={totals} /></DialogContent></Dialog>
...
function CartPanel({ cart, totals, changeQuantity, onRemove, onCheckout, onEditNote, canSave, currentOrder, onSave, onSend, pending, modal = false }: {
  cart: CartLine[];
  totals: ReturnType<typeof calculateClientTotals>;
