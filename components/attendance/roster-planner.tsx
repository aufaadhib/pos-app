...
import { Archive, CalendarPlus, ClipboardCopy, Clock3, Pencil, Plus, Save, Send } from "lucide-react";
import { toast } from "react-toastify";
import { addPublishedRosterEntryAction, archiveShiftTemplateAction, copyRosterWeekAction, createShiftTemplateAction, publishRosterWeekAction, saveRosterDraftAction, updatePublishedRosterEntryAction, updateShiftTemplateAction } from "@/app/attendance/roster/actions";
...
export function RosterPlanner({ outlet, staff, templates, week, weekStart }: { outlet: { id: string; code: string; name: string; timezone: string }; staff: Staff[]; templates: Template[]; week: Week; weekStart: string }) {
  ...
  const templateOptions = [{ value: DAY_OFF, label: "Libur · hapus shift" }, ...templates.map((template) => ({ value: template.id, label: `${template.name} · ${template.startTime}–${template.endTime}` }))];
  ...
  function beginRevision(userId: string, workDate: string, entry: Entry | null) {
    setRevision({ userId, workDate, entry });
    setRevisionTemplateId(entry?.shiftTemplateId ?? templates[0]?.id ?? "");
    setReason("");
  }
