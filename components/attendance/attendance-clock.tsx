...
export function AttendanceClock({ user, outlets, profile, openSession, recentSessions }: {
  ...
  return <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(18rem,0.85fr)] xl:items-start">
    ...
    <Dialog onOpenChange={(open) => { if (!open) closeDialog(); }} open={dialogMode !== null}><DialogContent className="sm:w-[min(38rem,calc(100vw-3rem))]"><DialogHeader><DialogTitle>{dialogMode === "enroll" ? "Daftarkan wajah akun" : openSession ? "Verifikasi absensi pulang" : "Verifikasi absensi masuk"}</DialogTitle><DialogDescription>{dialogMode === "enroll" ? "Ambil tiga sampel wajah terang dan tajam. Template disimpan terenkripsi." : challenge?.actionLabel ?? "Menyiapkan challenge liveness…"}</DialogDescription></DialogHeader>
      <div className="relative aspect-[4/3] overflow-hidden rounded-xl bg-black"><video aria-label="Pratinjau kamera absensi" className="h-full w-full scale-x-[-1] object-cover" muted playsInline ref={videoRef} />{cameraStatus !== "ready" && <div className="absolute inset-0 grid place-items-center bg-black/75 p-6 text-center text-white">{cameraStatus === "loading" ? <span className="grid justify-items-center gap-3"><Spinner className="size-7" />Memuat kamera dan model wajah…</span> : cameraStatus === "error" ? <span>Kamera atau model wajah tidak tersedia. Periksa izin kamera dan koneksi.</span> : null}</div>}<div className="pointer-events-none absolute inset-[12%] rounded-[45%] border-2 border-white/80 shadow-[0_0_0_999px_rgb(0_0_0/20%)]" /></div>
