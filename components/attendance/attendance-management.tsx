...
type PendingRequest = { id: string; reason: string; requestedAt: string; userId: string; user: { id: string; name: string; email: string }; verification: { kind: "CHECK_IN" | "CHECK_OUT" }; attempt: { id: string; attemptedAt: string; failureReason: string | null; evidenceDeletedAt: string | null } };
type ManagedSession = { id: string; userId: string; user?: { id?: string; name: string; email: string }; status: "OPEN" | "CLOSED"; checkInAt: string; checkOutAt: string | null; originalCheckInAt: string; originalCheckOutAt: string | null; outlet: { code: string; name: string; timezone: string }; correction: { reason: string; actorName: string; createdAt: string } | null };
...
export function AttendanceManagement({ outletId, currentUserId, pendingRequests, sessions, staffProfiles }: { outletId: string; currentUserId: string; pendingRequests: PendingRequest[]; sessions: ManagedSession[]; staffProfiles: StaffProfile[] }) {
  ...
  return <div className="grid gap-6">
    <section aria-labelledby="exception-heading"><div className="flex items-end justify-between gap-4"><div><h2 className="font-heading text-xl font-semibold" id="exception-heading">Pengecualian menunggu</h2><p className="mt-1 text-sm text-muted-foreground">Muncul setelah tiga kegagalan dalam 15 menit.</p></div><Badge variant={pendingRequests.length ? "destructive" : "secondary"}>{pendingRequests.length} pending</Badge></div>
      <div className="mt-4 grid gap-3 lg:grid-cols-2">{pendingRequests.length ? pendingRequests.map((request) => <article className="rounded-xl border bg-card p-4" key={request.id}><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{request.user.name}</h3><p className="text-sm text-muted-foreground">{request.user.email}</p></div><Badge variant="outline">{request.verification.kind === "CHECK_IN" ? "Masuk" : "Pulang"}</Badge></div><p className="mt-3 rounded-lg bg-muted/60 p-3 text-sm leading-6">{request.reason}</p><p className="mt-3 text-xs text-muted-foreground">Percobaan {formatDateTime(request.attempt.attemptedAt)} · {request.attempt.failureReason ?? "Gagal"}</p><div className="mt-4 flex flex-wrap gap-2"><Button className="min-h-11" disabled={request.attempt.evidenceDeletedAt !== null} nativeButton={false} render={<a href={`/api/attendance/evidence/${request.attempt.id}`} rel="noreferrer" target="_blank" />} variant="outline"><Eye aria-hidden="true" />Lihat bukti</Button><Button className="min-h-11" disabled={request.userId === currentUserId} onClick={() => { setReview(request); setReason(""); }} type="button"><ShieldCheck aria-hidden="true" />Tinjau</Button></div>{request.userId === currentUserId && <p className="mt-2 text-xs text-destructive">Permintaan sendiri harus ditinjau manager lain atau pemilik.</p>}</article>) : <div className="rounded-xl border border-dashed bg-card p-6 text-sm text-muted-foreground lg:col-span-2">Tidak ada permintaan pengecualian yang menunggu.</div>}</div>
    </section>

    <section aria-labelledby="records-heading"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="font-heading text-xl font-semibold" id="records-heading">Catatan kehadiran</h2><p className="mt-1 text-sm text-muted-foreground">Koreksi disimpan sebagai catatan baru tanpa mengubah waktu asli.</p></div><div className="flex flex-wrap items-end gap-2"><label className="grid gap-1 text-xs" htmlFor="attendance-export-from">Dari<Input className="h-10" id="attendance-export-from" onChange={(event) => setFrom(event.target.value)} type="date" value={from} /></label><label className="grid gap-1 text-xs" htmlFor="attendance-export-to">Sampai<Input className="h-10" id="attendance-export-to" onChange={(event) => setTo(event.target.value)} type="date" value={to} /></label><Button className="min-h-10" nativeButton={false} render={<a href={`/api/reports/attendance/export?outletId=${encodeURIComponent(outletId)}&from=${from}&to=${to}`} />} variant="outline"><Download aria-hidden="true" />CSV</Button></div></div>
      <div className="mt-4 hidden overflow-hidden rounded-xl border bg-card md:block"><Table><TableHeader><TableRow><TableHead>Staf</TableHead><TableHead>Masuk</TableHead><TableHead>Pulang</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Aksi</TableHead></TableRow></TableHeader><TableBody>{sessions.map((session) => <TableRow key={session.id}><TableCell><span className="font-semibold">{session.user?.name}</span><span className="block text-xs text-muted-foreground">{session.user?.email}</span></TableCell><TableCell>{formatDateTime(session.checkInAt)}</TableCell><TableCell>{session.checkOutAt ? formatDateTime(session.checkOutAt) : "—"}</TableCell><TableCell><Badge variant="outline">{session.status === "OPEN" ? "Terbuka" : session.correction ? "Dikoreksi" : "Selesai"}</Badge></TableCell><TableCell className="text-right"><Button className="min-h-10" onClick={() => openCorrection(session)} size="sm" type="button" variant="outline"><FilePenLine aria-hidden="true" />Koreksi</Button></TableCell></TableRow>)}</TableBody></Table></div>
      <div className="mt-4 grid gap-3 md:hidden">{sessions.map((session) => <article className="rounded-xl border bg-card p-4" key={session.id}><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{session.user?.name}</h3><p className="text-xs text-muted-foreground">{session.user?.email}</p></div><Badge variant="outline">{session.status === "OPEN" ? "Terbuka" : "Selesai"}</Badge></div><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-xs text-muted-foreground">Masuk</dt><dd className="mt-1">{formatDateTime(session.checkInAt)}</dd></div><div><dt className="text-xs text-muted-foreground">Pulang</dt><dd className="mt-1">{session.checkOutAt ? formatDateTime(session.checkOutAt) : "—"}</dd></div></dl><Button className="mt-4 min-h-11 w-full" onClick={() => openCorrection(session)} type="button" variant="outline"><FilePenLine aria-hidden="true" />Koreksi waktu</Button></article>)}</div>
