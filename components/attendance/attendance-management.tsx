...
type PendingRequest = { id: string; reason: string; requestedAt: string; userId: string; user: { id: string; name: string; email: string }; verification: { kind: "CHECK_IN" | "CHECK_OUT" }; attempt: { id: string; attemptedAt: string; failureReason: string | null; similarity: string | null; evidenceAvailable: boolean } };
type AttendanceEvidence = { attemptId: string; available: boolean; similarity: string | null } | null;
type ManagedSession = { id: string; userId: string; user?: { id?: string; name: string; email: string }; status: "OPEN" | "CLOSED"; checkInAt: string; checkOutAt: string | null; missedCheckout: boolean; originalCheckInAt: string; originalCheckOutAt: string | null; outlet: { code: string; name: string; timezone: string }; checkInEvidence: AttendanceEvidence; checkOutEvidence: AttendanceEvidence; correction: { reason: string; actorName: string; createdAt: string } | null };
...
export function AttendanceManagement({ outletId, currentUserId, pendingRequests, sessions, staffProfiles, timezone }: { outletId: string; currentUserId: string; pendingRequests: PendingRequest[]; sessions: ManagedSession[]; staffProfiles: StaffProfile[]; timezone: string }) {
  ...
  return <div className="grid gap-6">
    <section aria-labelledby="exception-heading"><div className="flex items-end justify-between gap-4"><div><h2 className="font-heading text-xl font-semibold" id="exception-heading">Pengecualian menunggu</h2><p className="mt-1 text-sm text-muted-foreground">Muncul setelah tiga kegagalan dalam 15 menit.</p></div><Badge variant={pendingRequests.length ? "destructive" : "secondary"}>{pendingRequests.length} pending</Badge></div>
      <div className="mt-4 grid gap-3 lg:grid-cols-2">{pendingRequests.length ? pendingRequests.map((request) => <article className="rounded-xl border bg-card p-4" key={request.id}><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{request.user.name}</h3><p className="text-sm text-muted-foreground">{request.user.email}</p></div><Badge variant="outline">{request.verification.kind === "CHECK_IN" ? "Masuk" : "Pulang"}</Badge></div><p className="mt-3 rounded-lg bg-muted/60 p-3 text-sm leading-6">{request.reason}</p><p className="mt-3 text-xs text-muted-foreground">Percobaan {formatDateTime(request.attempt.attemptedAt, timezone)} · {request.attempt.failureReason ?? "Gagal"}{request.attempt.similarity ? ` · Kecocokan ${formatSimilarity(request.attempt.similarity)}` : ""}</p><div className="mt-4 flex flex-wrap gap-2"><AttendanceEvidenceButton evidence={{ attemptId: request.attempt.id, available: request.attempt.evidenceAvailable, similarity: request.attempt.similarity }} label="Lihat bukti" /><Button className="min-h-11" disabled={request.userId === currentUserId} onClick={() => { setReview(request); setReason(""); }} type="button"><ShieldCheck aria-hidden="true" />Tinjau</Button></div>{request.userId === currentUserId && <p className="mt-2 text-xs text-destructive">Permintaan sendiri harus ditinjau manager lain atau pemilik.</p>}</article>) : <div className="rounded-xl border border-dashed bg-card p-6 text-sm text-muted-foreground lg:col-span-2">Tidak ada permintaan pengecualian yang menunggu.</div>}</div>
    </section>

    <section aria-labelledby="records-heading"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><h2 className="font-heading text-xl font-semibold" id="records-heading">Catatan kehadiran</h2><p className="mt-1 text-sm text-muted-foreground">Koreksi disimpan sebagai catatan baru tanpa mengubah waktu asli.</p></div><div className="grid w-full min-w-0 grid-cols-1 items-end gap-2 sm:max-w-xl sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] lg:w-auto"><label className="grid min-w-0 gap-1 text-xs" htmlFor="attendance-export-from">Dari<Input className="ios-date-input h-10 min-w-0 max-w-full" id="attendance-export-from" onChange={(event) => setFrom(event.target.value)} type="date" value={from} /></label><label className="grid min-w-0 gap-1 text-xs" htmlFor="attendance-export-to">Sampai<Input className="ios-date-input h-10 min-w-0 max-w-full" id="attendance-export-to" onChange={(event) => setTo(event.target.value)} type="date" value={to} /></label><Button className="min-h-11 w-full sm:w-auto" nativeButton={false} render={<a href={`/api/reports/attendance/export?outletId=${encodeURIComponent(outletId)}&from=${from}&to=${to}`} />} variant="outline"><Download aria-hidden="true" />CSV</Button></div></div>
      <div className="mt-4 hidden overflow-hidden rounded-xl border bg-card lg:block"><Table><TableHeader><TableRow><TableHead>Staf</TableHead><TableHead>Masuk</TableHead><TableHead>Pulang</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Aksi</TableHead></TableRow></TableHeader><TableBody>{sessions.map((session) => <TableRow key={session.id}><TableCell><span className="font-semibold">{session.user?.name}</span><span className="block text-xs text-muted-foreground">{session.user?.email}</span></TableCell><TableCell>{formatDateTime(session.checkInAt, session.outlet.timezone)}{session.checkInEvidence?.similarity && <span className="block text-xs text-muted-foreground">Wajah {formatSimilarity(session.checkInEvidence.similarity)}</span>}</TableCell><TableCell>{session.checkOutAt ? formatDateTime(session.checkOutAt, session.outlet.timezone) : "—"}{session.checkOutEvidence?.similarity && <span className="block text-xs text-muted-foreground">Wajah {formatSimilarity(session.checkOutEvidence.similarity)}</span>}</TableCell><TableCell><Badge variant={session.missedCheckout ? "destructive" : "outline"}>{session.missedCheckout ? "Tidak absen pulang" : session.status === "OPEN" ? "Terbuka" : session.correction ? "Dikoreksi" : "Selesai"}</Badge></TableCell><TableCell><div className="flex flex-wrap justify-end gap-2"><AttendanceEvidenceButton evidence={session.checkInEvidence} label="Foto masuk" /><AttendanceEvidenceButton evidence={session.checkOutEvidence} label="Foto pulang" /><Button className="min-h-11" onClick={() => openCorrection(session)} size="sm" type="button" variant="outline"><FilePenLine aria-hidden="true" />Koreksi</Button></div></TableCell></TableRow>)}</TableBody></Table></div>
      <div className="mt-4 grid gap-3 lg:hidden">{sessions.map((session) => <article className="rounded-xl border bg-card p-4" key={session.id}><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{session.user?.name}</h3><p className="text-xs text-muted-foreground">{session.user?.email}</p></div><Badge variant={session.missedCheckout ? "destructive" : "outline"}>{session.missedCheckout ? "Tidak absen pulang" : session.status === "OPEN" ? "Terbuka" : "Selesai"}</Badge></div><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-xs text-muted-foreground">Masuk</dt><dd className="mt-1">{formatDateTime(session.checkInAt, session.outlet.timezone)}{session.checkInEvidence?.similarity && <span className="block text-xs text-muted-foreground">Wajah {formatSimilarity(session.checkInEvidence.similarity)}</span>}</dd></div><div><dt className="text-xs text-muted-foreground">Pulang</dt><dd className="mt-1">{session.checkOutAt ? formatDateTime(session.checkOutAt, session.outlet.timezone) : session.missedCheckout ? "Tidak tercatat" : "—"}{session.checkOutEvidence?.similarity && <span className="block text-xs text-muted-foreground">Wajah {formatSimilarity(session.checkOutEvidence.similarity)}</span>}</dd></div></dl><div className="mt-4 grid grid-cols-2 gap-2"><AttendanceEvidenceButton evidence={session.checkInEvidence} label="Foto masuk" /><AttendanceEvidenceButton evidence={session.checkOutEvidence} label="Foto pulang" /><Button className="col-span-2 min-h-11 w-full" onClick={() => openCorrection(session)} type="button" variant="outline"><FilePenLine aria-hidden="true" />Koreksi waktu</Button></div></article>)}</div>
...
/** Formats a stored cosine similarity as a compact calibration percentage. */
function formatSimilarity(value: string) {
  return new Intl.NumberFormat("id-ID", { style: "percent", maximumFractionDigits: 1 }).format(Number(value));
}
