...
| Schema roster | Mode, pola tetap, template/draf/publish/salin/revisi/tambah | Input tervalidasi | Membatasi mode per outlet, weekday, jam, tanggal minggu, satu shift per staf/tanggal, versi optimistic, dan alasan perubahan jadwal terbit. |
...
| `saveFixedSchedules()`, `updateScheduleMode()` | Outlet, pola staf/hari, mode, versi, actor | Konfigurasi terbaru | Menyimpan pola berulang, mewajibkan jadwal setiap staf saat aktivasi, mempertahankan roster terbit, dan mencatat audit atomik. |
| `materializeUpcomingFixedRosters()`, `ensureFixedRosterWeek()` | Waktu atau transaction absensi | Jumlah/materialisasi minggu | Membuat snapshot roster tetap minggu berjalan dan berikutnya secara idempoten melalui cron serta fallback check-in. |
| `resetFixedScheduleOverride()` | Staf, tanggal, minggu, alasan | Jadwal pola atau Libur | Menghapus pengecualian satu tanggal dan mengembalikan sel ke pola tetap tanpa mengubah riwayat terbit. |
...
| `RosterPlanner()` | Outlet, staf, mode, pola, template, minggu | Pemilih mode dan papan/kartu roster | Menyediakan matriks pola tetap, salin massal, draf/publish manual, override tanggal, papan desktop, dan kartu hari tablet/mobile tanpa overflow horizontal. |
