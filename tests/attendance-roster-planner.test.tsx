...
vi.mock("@/app/attendance/roster/actions", () => ({
  ...
  saveFixedSchedulesAction: actions.saveFixed,
  resetFixedScheduleOverrideAction: actions.resetFixed,
  updateScheduleModeAction: actions.updateMode,
...
const outlet = { id: "outlet-1", code: "JKT", name: "Jakarta", timezone: "Asia/Jakarta", attendanceScheduleMode: "WEEKLY" as const, attendanceScheduleEffectiveFrom: null, updatedAt: "2026-08-11T01:00:00.000Z" };
...
const week = { id: "week-1", status: "PUBLISHED" as const, source: "MANUAL" as const, publishedAt: "2026-08-11T01:00:00.000Z", updatedAt: "2026-08-11T01:00:00.000Z", entries: [{ id: "entry-1", userId: "staff-1", workDate: "2099-08-10", shiftTemplateId: "shift-1", shiftName: "Pagi", scheduledStartAt: "2099-08-10T01:00:00.000Z", scheduledEndAt: "2099-08-10T09:00:00.000Z", updatedAt: "2026-08-11T01:00:00.000Z" }] };
...
describe("responsive roster planner", () => {
  ...
  it("opens template editing and confirms archival from the active template card", async () => {
    ...
    render(<RosterPlanner fixedSchedules={[]} outlet={outlet} overrides={[]} staff={staff} templates={templates} week={week} weekStart="2099-08-10" />);
  ...
  it("copies one fixed pattern to another staff member and enables fixed mode", async () => {
    const user = userEvent.setup();
    const team = [...staff, { id: "staff-2", name: "Bima", email: "bima@example.com", role: "staff", jobPosition: { id: "position-1", name: "Pelayan" } }];
    render(<RosterPlanner fixedSchedules={[{ userId: "staff-1", weekday: 1, shiftTemplateId: "shift-1" }]} outlet={outlet} overrides={[]} staff={team} templates={templates} week={null} weekStart="2099-08-10" />);

    expect(screen.queryByRole("heading", { name: "Pola jadwal tetap" })).not.toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Siapkan jadwal tetap" }));
    expect(screen.getByRole("heading", { name: "Pola jadwal tetap" })).toBeVisible();
    expect(screen.getByRole("button", { name: "Gunakan jadwal tetap" })).toBeDisabled();
    await user.click(screen.getByRole("button", { name: "Salin pola" }));
    await user.click(within(screen.getByRole("dialog")).getByRole("checkbox"));
    await user.click(within(screen.getByRole("dialog")).getByRole("button", { name: "Salin pola" }));
    expect(screen.getByRole("button", { name: "Gunakan jadwal tetap" })).toBeEnabled();
    await user.click(screen.getByRole("button", { name: "Simpan pola" }));
