...
export default async function OutletsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <div className="workspace-shell min-h-svh bg-background">
      <WorkspaceHeader activeRoute="outlets" activeOutletId={session.session.activeOutletId} canManageStaff={role !== "cashier"} canViewDesignSystem={role === "owner"} role={role} />
