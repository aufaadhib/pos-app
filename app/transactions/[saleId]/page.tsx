...
export default async function SaleDetailPage({ params }: { params: Promise<{ saleId: string }> }) {
  ...
  return <main className="mx-auto max-w-3xl px-4 py-6 sm:px-8 sm:py-8" id="main-content">
    ...
    <article className="overflow-hidden rounded-2xl border border-t-4 border-t-primary bg-card">
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3 border-t p-5 text-xs text-muted-foreground sm:px-7"><span className="flex items-center gap-3"><ReceiptText aria-hidden="true" className="size-4 shrink-0" />{sale.outletName} · {sale.outletCode} · dilayani {sale.createdByName}</span>{sale.shiftId ? <Link className="font-semibold text-foreground underline underline-offset-4" href={`/shifts/${sale.shiftId}`}>Lihat shift</Link> : <Badge variant="outline">Transaksi lama tanpa shift</Badge>}</footer>
    </article>
  </main>;
}
