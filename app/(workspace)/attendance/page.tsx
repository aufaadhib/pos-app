...
import { CalendarRange, Settings2, UsersRound } from "lucide-react";
...
export default async function AttendancePage() {
  ...
  const canManage = roleHasPermission(session.user.role, { attendance: ["manage"] });
  return <main className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-8 sm:py-8 lg:px-10" id="main-content">
    <header className="mb-5 flex flex-col gap-4 rounded-2xl border bg-card p-5 sm:p-6 lg:flex-row lg:items-end lg:justify-between">
      ...
      {canManage && <div className="flex flex-wrap gap-2"><Link className={buttonVariants({ variant: "outline", className: "min-h-11" })} href="/settings/attendance"><Settings2 aria-hidden="true" />Atur lokasi</Link><Link className={buttonVariants({ variant: "outline", className: "min-h-11" })} href="/attendance/roster"><CalendarRange aria-hidden="true" />Roster</Link><Link className={buttonVariants({ className: "min-h-11" })} href="/attendance/manage"><UsersRound aria-hidden="true" />Kelola absensi</Link></div>}
    </header>
    <AttendanceClock openSession={data.openSession} outlets={data.outlets} pendingReenrollment={data.pendingReenrollment} profile={data.profile} recentSessions={data.recentSessions} roster={data.roster} user={{ name: session.user.name, email: session.user.email, role: session.user.role }} />
  </main>;
}
